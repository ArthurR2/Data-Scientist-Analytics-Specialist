...
-- Also
SELECT playlist_track.TrackId, tracks.Name, COUNT(Tracks.TrackId)
FROM playlist_track
LEFT JOIN tracks
ON playlist_track.TrackId = tracks.TrackId
GROUP BY 1
...
-- ALSO
SELECT tracks.Name, SUM(invoice_items.UnitPrice)
FROM tracks
JOIN invoice_items
ON tracks.TrackId = invoice_items.TrackId
GROUP BY 1
...
-- also 
SELECT Title, SUM(invoice_items.UnitPrice)
FROM invoice_items
JOIN tracks
ON tracks.TrackId = invoice_items.TrackId
JOIN albums
...
COUNT(customers.CustomerId), AVG(invoices.Total), SUM(invoices.Total)
FROM employees
JOIN customers
ON employees.EmployeeId = customers.SupportRepId
JOIN invoices
